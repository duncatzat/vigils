...
- ✅ `content-script.js`:4 个 host(ChatGPT / Claude / Gemini / Perplexity)paste + 防抖 input + submit + contenteditable Enter 路径 + 简易 `textContent` toast;submit allow 走 `form.requestSubmit(submitter)` + `WeakSet` allow-once(R1 MUST-FIX 1 保留 HTML validation + 其他 submit listener 参与)
...
3. 访问 4 host 之一,touch paste / input / submit → 若 Native Host 未注册,应看到 "Vigil: 粘贴被阻断(host_disconnected)" 或输入/提交阻断提示(fail-closed)
