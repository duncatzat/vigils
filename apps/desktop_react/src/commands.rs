...
// 2. `apps/desktop_react/capabilities/default.json` → 引用 `allow-{slugified}` 把命令纳入
//    主窗口 capability(无前缀 = APP_ACL_KEY,tauri-utils::resolved.rs L344 语义)
//
// 本列表必须与 `apps/desktop_react/src/main.rs` 的 `tauri::generate_handler![...]` 严格一致。
...
///
/// **顺序不重要**(内部按 slugified 生成 permission);共 **30** 条
/// (α1=1 + α2=3 + α3=5 + ONNX=2 + α4=11 + α5=2 + ISS-017=1 + ISS-018=1 + D19=1 + Sandbox=3)。
#[allow(dead_code)]
pub(crate) const INVOKE_COMMANDS: &[&str] = &[
    // α1(Sessions smoke)
    "list_sessions",
    ...
    "get_approval_detail",
    "resolve_approval",
    // α3(Activity Feed 全套 —— 3 handler + checkpoint anchor + update_hub_config)
    "list_recent_events",
    "get_event_detail",
    "fts_search",
    "anchor_checkpoint",
    "update_hub_config",
    // ONNX Model Management(2 handler)
    "list_onnx_models",
    "ensure_onnx_model",
    ...
    fn invoke_commands_count_in_sync() {
        assert_eq!(
            ...
            "INVOKE_COMMANDS 漂移 —— 新增/删除 handler 时必须同步:\n\
             1) 本文件 `apps/desktop_react/src/commands.rs`\n\
             2) `apps/desktop_react/src/main.rs` 的 `tauri::generate_handler!` 列表\n\
             3) `apps/desktop_react/capabilities/default.json` 的 `allow-$cmd` permission 清单"
        );
    }
