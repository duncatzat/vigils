//! Vigils Desktop React — Tauri 2 GUI 二进制入口。
//!
//! # 架构
//!
...
use vigil_mcp::hub::HubConfigPatch;
use vigil_ui_protocol::{
    ApprovalDetailDto, ApprovalResolutionDto, ApprovalSummary, ApproveServerCommandDriftReq,
    ApproveToolDriftReq, ApproveToolReq, Capability, ChainVerifyReport, CheckpointAnchorDto,
    EnsureOnnxModelReq, EventDetail, EventSummary, ExportSessionReplayReq, FtsSearchReq,
    ...
    OnnxModelInfo, PrivacyFindingsDto, RegisterServerReq, RejectServerCommandDriftReq,
    RejectToolDriftReq, ReplaySessionReq, ResolveApprovalReq, SandboxProfileUpsertDto,
    SessionExportDto, SessionReplay, UiCommand, UiResponse, UpdateHubConfigReq,
...
/// `invoke('register_server', { req })` → UiCommand::RegisterServer → Ledger.register_server。
/// 新增 MCP server 登记(写命令)。
#[tauri::command]
async fn register_server(
    req: RegisterServerReq,
    state: State<'_, AppState>,
) -> Result<(), String> {
    let resp = dispatch(
        UiCommand::RegisterServer(req),
        state.ledger.as_ref(),
        Capability::Write,
    )
    .map_err(|e| e.to_string())?;
    match resp {
        UiResponse::Ack => Ok(()),
        other => Err(format!(
            "unexpected response shape for register_server: {other:?}"
        )),
    }
}

/// `invoke('list_sandbox_profiles')` → UiCommand::ListSandboxProfiles。
#[tauri::command]
async fn list_sandbox_profiles(state: State<'_, AppState>) -> Result<Vec<SandboxProfile>, String> {
    let resp = dispatch(
    ...
    .map_err(|e| e.to_string())?;
    match resp {
        UiResponse::SandboxProfileList(rows) => Ok(rows),
        other => Err(format!(
            "unexpected response shape for list_sandbox_profiles: {other:?}"
        )),
    }
}

/// `invoke('get_sandbox_profile', { req })` → UiCommand::GetSandboxProfile。
#[tauri::command]
async fn get_sandbox_profile(
    req: GetSandboxProfileReq,
    state: State<'_, AppState>,
    ...
    .map_err(|e| e.to_string())?;
    match resp {
        UiResponse::SandboxProfileOpt(opt) => Ok(opt),
        other => Err(format!(
            "unexpected response shape for get_sandbox_profile: {other:?}"
        )),
    }
}

/// `invoke('upsert_sandbox_profile', { req })` → UiCommand::UpsertSandboxProfile(写命令)。
#[tauri::command]
async fn upsert_sandbox_profile(
    req: UpsertSandboxProfileReq,
    state: State<'_, AppState>,
) -> Result<SandboxProfileUpsertDto, String> {
    let resp = dispatch(
    ...
    .map_err(|e| e.to_string())?;
    match resp {
        UiResponse::SandboxProfileUpserted(dto) => Ok(dto),
        other => Err(format!(
            "unexpected response shape for upsert_sandbox_profile: {other:?}"
        )),
    }
}
...
/// `invoke('anchor_checkpoint')` → UiCommand::AnchorCheckpoint(写命令)。
#[tauri::command]
async fn anchor_checkpoint(state: State<'_, AppState>) -> Result<Option<CheckpointAnchorDto>, String> {
    let resp = dispatch(
    ...
    .map_err(|e| e.to_string())?;
    match resp {
        UiResponse::CheckpointAnchored(dto) => Ok(dto),
        other => Err(format!(
            "unexpected response shape for anchor_checkpoint: {other:?}"
        )),
    }
}

/// `invoke('update_hub_config', { req })` → 热更新运行中 embed Hub 的保护策略。
///
/// 直接操作 `State<'_, Arc<Hub>>`,不走 dispatch(与 `resolve_approval` 同理:
/// Hub 层配置变更自然在 Hub 上完成)。写命令,Capability::Write。
#[tauri::command]
async fn update_hub_config(
    req: UpdateHubConfigReq,
    hub: State<'_, Arc<Hub>>,
) -> Result<(), String> {
    let patch = HubConfigPatch {
        monitor_mode: req.monitor_mode,
        auto_approve_first_seen_tools: req.auto_approve_first_seen_tools,
        redact_tool_results: req.redact_tool_results,
        outbox_enabled: req.outbox_enabled,
        approval_wait_secs: req.approval_wait_secs,
        upstream_list_timeout_secs: req.upstream_list_timeout_secs,
        upstream_call_timeout_secs: req.upstream_call_timeout_secs,
    };
    hub.update_config(patch).map_err(|e| e.to_string())
}

/// `invoke('list_onnx_models')` → 返回本地 ONNX 模型安装状态(只读)。
///
/// 当前管理两类模型:
/// - `privacy-filter`:OpenAI Privacy Filter(PII NER)
/// - `deberta-injection`:prompt-injection 分类器
///
/// 只检查三件套存在性 + sha256,**不**触发下载;零回显(只返回元数据)。
#[tauri::command]
async fn list_onnx_models() -> Result<Vec<OnnxModelInfo>, String> {
    use vigil_redaction::bootstrap::{
        injection_classifier_manifest, placeholder_manifest, verify,
    };

    fn info_for(
        model_id: &str,
        display_name: &str,
        manifest: &vigil_redaction::bootstrap::Manifest,
    ) -> OnnxModelInfo {
        let target_dir = dirs::data_local_dir()
            .map(|p| p.join("vigil").join("models").join(format!(
                "{}-{}",
                manifest.model_name, manifest.version
            )))
            .unwrap_or_default();
        let installed = verify::check_existing(&target_dir, manifest).is_some();
    ...
    Ok(vec![
        ...
        info_for(
            ...
            &injection_classifier_manifest(),
        ),
    ])
}

/// ONNX 模型下载进度事件 payload。
#[derive(Clone, serde::Serialize)]
struct OnnxModelProgress {
    model_id: String,
    phase: String,
    message: Option<String>,
}

/// `invoke('ensure_onnx_model', { req })` → 在后台线程触发模型下载/校验。
///
/// 立即返回 `job_id`,caller 监听 `onnx-model-progress` 事件获取开始/完成/失败通知。
/// 写命令,Capability::Write。失败原因经脱敏处理(不暴露本地路径)。
#[tauri::command]
async fn ensure_onnx_model(
    req: EnsureOnnxModelReq,
    app: tauri::AppHandle,
    ...
    std::thread::spawn(move || {
        let result: Result<vigil_redaction::bootstrap::ModelPaths, String> = match model_id.as_str() {
            "privacy-filter" => {
                vigil_redaction::bootstrap::ensure_model_available(None)
                    .map_err(|e| e.to_string())
            }
            "deberta-injection" => vigil_redaction::bootstrap::ensure_injection_model_available(None)
                .map_err(|e| e.to_string()),
            other => Err(format!("unknown model_id: {other}")),
        };

        let (phase, message) = match result {
            Ok(_) => ("completed".to_string(), None),
            Err(e) => ("failed".to_string(), Some(sanitize_bootstrap_error(&e))),
        };
        let _ = app.emit(
            "onnx-model-progress",
            OnnxModelProgress {
...
/// 对 bootstrap 错误做最小脱敏:保留错误类别,去掉可能含路径的原始文本。
fn sanitize_bootstrap_error(e: &str) -> String {
    if e.contains("sha256 mismatch") {
        "sha256 mismatch".to_string()
    } else if e.contains("download failed") || e.contains("all mirrors unreachable") {
        "network/download failed".to_string()
    } else if e.contains("disk full") || e.contains("io error") {
        "disk/io error".to_string()
    } else {
...
fn main() {
    ...
    tauri::Builder::default()
        ...
        .invoke_handler(tauri::generate_handler![
            ...
            get_sandbox_profile,
            upsert_sandbox_profile,
            // α4(Server Registry 全套 —— 5 read + 6 write)
            register_server,
            list_servers,
