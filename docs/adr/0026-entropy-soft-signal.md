...
1. **`HIGH_ENTROPY_RISK_DELTA` 档位**:提案 5(最弱档)。若实测显示精度远高于预期,可上调到 8
   与元指令齐平;但**不应**达到 10,那是有确定语义的 Email/Url 档。
2. **阈值表来源**:Cosy 的 `BIGRAM_COST`(37×37)与 17 锚点 `ENTROPY_THRESHOLDS` 是 MIT,
   **可直接 vendor(须保留版权声明)**作为起点;或用本项目自己的语料重算。
   前者快且有实证来源,后者更贴合我们的输入分布。建议:**先 vendor 跑通 + 出基线,再决定是否重算**。
3. **上哪几条腿**:提案 hook + MCP 双腿(与 `scan_meta_instructions` 完全一致)。
   出站腿**不上**(§1.2 / §3)。
4. **`FindingSource::Entropy` 是否值得新增 variant**:新增会触发
   `finding_source_variants_exhaustive_guard` 编译期提醒(这是**好事**,强制所有消费点同步);
   但也需同步 `vigil-sdk` re-export 契约与所有 `match f.source` 处。替代方案是复用
   `MetaInstruction` —— **不推荐**,那会让两类语义完全不同的信号在审计里无法区分。
5. **是否引入 nuisance 方向投影**(见 §3):它直接对着 §3 列出的 base64 / UUID / git SHA /
   minified 误报面,但会把实现从「一张 bigram 代价表 + 两道门限」升级成「**先估计编码区制、
   再判异常**」,复杂度与可解释性成本都显著上升。**建议:先按 D5 的双门限跑出真实误报
   分布,拿到数据后再决定是否值得** —— 在没有自己的基线之前,任何降误报手段都无从比较。
