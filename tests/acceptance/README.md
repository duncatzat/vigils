...
build-provenance attestation, then runs `user-sim.sh`, `functional-sweep.sh`, `core-e2e.mjs`, `http-e2e.mjs`, `outbound-e2e.mjs` and `agent-compat.sh`; a fourth
...
| 2 Core-function e2e | `core-e2e.mjs` (+ `mock-upstream.mjs`) | any `vigil-hub` (`HUB=…`), node | **privacy filter + secret lease on the real MCP gateway**: drives a real stdio MCP client through `serve --stdio` with a mock upstream — `secret://<alias>` four-leg (agent writes placeholder → gateway injects the real **ghp_ fingerprint** value at the tool boundary, upstream sideband confirms receipt, client result carries no plaintext), leaky-upstream **result re-redaction**, raw-secret arg **not forwarded**, and **no plaintext anywhere in the ledger**. Secret source is `env:` (no OS-keyring cross-language issues) |
| 2 Remote-MCP e2e | `http-e2e.mjs` (+ `mock-http-upstream.mjs`) | any `vigil-hub` (`HUB=…`), node | same protection invariants pushed onto the **Streamable HTTP upstream** path (`{name,url,auth}` serve config): bearer token (`env:` source) reaches the upstream `Authorization` header but never the client/ledger, secret-lease four-leg over HTTP, **SSE response collapse + re-redaction** (leaky upstream answers as `text/event-stream`), raw-secret arg not forwarded, no plaintext (lease/bearer/raw) in the ledger. Gracefully SKIPs on builds pre-dating HTTP upstream support |
| 2 Outbound-gate e2e | `outbound-e2e.mjs` | any `vigil-hub` (`HUB=…`), node | the **opt-in outbound LLM-API gate** on the path a user actually takes — on-disk `outbound.json` → `outbound serve` → the published binary → real TCP → a real upstream (`gate_e2e.rs` builds `GateConfig` in-process and never exercises that path). Verdict is always **the bytes the upstream received**: hard-fingerprint matrix (`github_token` / `aws_access_key_id` / `anthropic_api_key` / `openai_api_key`) redacted before egress; `ghp_`+36 hits vs +34 misses (the threshold is settled semantics, not luck); clean bodies **not** false-positived; auth header forwarded verbatim; the three refusals (415 non-JSON / 403 browser `Origin` / 404 unknown prefix) **never egress**; `healthz` self-report `rewritten` **cross-checked against** upstream-observed redactions (two independent observation paths must agree); **exactly N** upstream requests (more = a refusal leaked, fewer = the harness broke); no credential in the gate's own log. Gracefully SKIPs on builds pre-dating the gate |
| 2 Agent-integration compat | `agent-compat.sh` | any `vigil-hub` (`HUB=…`) | Vigil hook honors **each agent's native hook protocol** so interception works when actually wired into Claude Code / Codex / Gemini / Cursor: Claude `PreToolUse`→exit 2; Codex `permissionDecision=deny`; Gemini `decision=deny`; Cursor `beforeShellExecution`→`permission=deny` — each fed the agent's real event shape, asserting deny + no raw-value echo + clean input not over-blocked. No LLM (protocol-contract layer) |
...
(`daemon.json`) and `outbound-e2e.mjs` (`outbound.json`) snapshot the one file they must
write, restore its original bytes on exit, and remove **only** artifacts the run created — never
the shared `%LOCALAPPDATA%\Vigil` dir (NTFS is case-insensitive: `Vigil` == `vigil`).
`outbound-e2e.mjs` additionally points `VIGIL_LEDGER_PATH` at its sandbox so the audit ledger
never lands in the real data dir, and `outbound serve` is **not** an update-check entry point
(only `serve --stdio` and `daemon start` are), so the run emits no version ping.
